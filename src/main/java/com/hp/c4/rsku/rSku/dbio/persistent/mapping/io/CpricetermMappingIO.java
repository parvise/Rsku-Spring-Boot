...
public class CpricetermMappingIO extends ObjectIO {
	...
	private static final String QUERY_SP_GETSEQUENCE = "c4output.selectIncotermSequence";
	...
	public List<String> loadAllSequenceIncoTermList() throws C4Exception {
		try {
			return (List<String>) callSp(getSelectProcedureInfo(QUERY_SP_GETSEQUENCE));
		} catch (SQLException e) {
			e.printStackTrace();
			throw new C4Exception(e.getMessage());
		}
	...
	public Object processResult(CProcedureInfo info) throws java.sql.SQLException, C4Exception {
		...
		if (info.getName().equals(SP_GETMAPPING)) {
			...
			return mappings;
		} else if (info.getName().equals(QUERY_SP_GETSEQUENCE)) {
			List<String> sequence = new ArrayList<String>();
			while (rs.next()) {
				sequence.add(rs.getString("ELEMENT_TYPE"));
			}
			return sequence;
		}
